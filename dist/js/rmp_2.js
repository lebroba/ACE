...
var r = Snap("#mobosvg");
$(function () {
	$("#mobosvg").width(high).height(high);
});
...
function osCircle() {
	...
	r.circle(center, center, radius + 2).attr(osParams);
	//race track Circle
	r.circle(center, center, radius * 2.5).attr(thickParams);
	r.polygon(10,10,10,30,100,100).attr({ fill: "red"});
	...
	for (var i = 0; i < 36; i++) {
		...
		label1 = r.text((center - 6 + ((radius - 20) * sin)), (center2 + 4 + ((radius - 20) * cos)), [
            [Math.round(theta * (180 / Math.PI), 0)]
        ]).attr(textParams1);
	}

	//Own Ship Inner Circle
	r.circle(center, center2, radius - 12).attr(osParams);
